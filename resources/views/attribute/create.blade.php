...
<div class="grid grid-cols-[256px_1fr] gap-4">
    ...
    <div class="p-5 mr-10 mt-8">
        ...
        <div class="flex flex-col dark:bg-white border-md border-solid rounded-md w-full p-3 mt-5">
            <div class="m-3">
                <form method="POST" action="{{ route('attribute.store') }}" class="space-y-4">
                    @csrf
                    ...
                    <!-- Row 2 -->
                    <div class="flex items-center gap-4">
                        <label for="type" class="w-40 text-gray-700 after:ml-0.5 after:text-red-500 after:content-['*']">
                            Attribute Type:
                        </label>
                        <select id="type" name="type" class="w-[40%] rounded-md border border-gray-300 px-3 py-2 placeholder-gray-400 shadow-sm
                        focus:border-sky-500 focus:outline focus:outline-sky-500 sm:text-sm" />
